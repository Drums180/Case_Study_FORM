...
colores_cluster <- c("red", "green", "blue")

# Seleccionar el tamaño de la muestra
tamano_muestra <- 20

# Crear una lista para almacenar las gráficas
plots <- list()

# Generar las gráficas por cada cluster
for (cluster_id in unique(resultados$Cluster)) {
  datos_cluster <- subset(resultados, Cluster == cluster_id)
  muestra <- datos_cluster[sample(nrow(datos_cluster), tamano_muestra), ]
  muestra$Caso <- 1:nrow(muestra)  # Añadir identificador de caso
  
  # Reestructurar los datos para la gráfica de líneas
  muestra_melt <- melt(muestra, id.vars = c("Caso", "Cluster"), measure.vars = c("Valores_Reales", "Predicciones"))
  
  # Crear la gráfica de líneas comparativa para el cluster
  plot <- ggplot(muestra_melt, aes(x = Caso, y = value, color = variable, group = variable)) +
    geom_line() +
    geom_point() +
    labs(title = paste("Comparación de Predicciones vs Valores Reales - Cluster", cluster_id),
         x = "Caso",
         y = "Cantidad",
         color = "Tipo") +
    theme_minimal() +
  ...
  plots[[as.numeric(cluster_id)]] <- plot
}

# Mostrar la gráfica para el Cluster 1
plots[[1]]
plots[[2]]
plots[[3]]
```
